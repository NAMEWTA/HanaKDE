...
固定审查 SHA/文件快照和合同后再派单。返回 `<Path>temp/subagent-delivery/{scope-id}/{task-id}/{dispatch-id}/inbound/attempt-NN/RETURN.md</Path>` 与 `<Path>temp/subagent-delivery/{scope-id}/{task-id}/{dispatch-id}/inbound/attempt-NN/FINDINGS.md</Path>`，每条 finding 包含严重度、文件/符号/行定位、触发条件、证据、影响、建议和置信度。不存在可定位证据的风格偏好不得冒充缺陷。
...
`<Path>temp/subagent-delivery/{scope-id}/{task-id}/{dispatch-id}/inbound/attempt-NN/SOURCES.json</Path>` 中每个来源至少记录 `url`、`title`、`publisher`、`published_or_updated`、`accessed_at`、`claims` 和 `limitations`。
...
2. Lead 创建 `<Path>temp/subagent-delivery/{scope-id}/{task-id}/{dispatch-id}/inbound/attempt-NN/staging/RETURN.md</Path>`，记录原始响应定位、dispatch identity、缺失字段和捕获方式；
