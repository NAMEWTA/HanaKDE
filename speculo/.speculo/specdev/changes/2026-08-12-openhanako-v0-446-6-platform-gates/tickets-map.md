...
| T-23 | `<Path>{roots.state}/specdev/changes/{change}/ticket/23-macos-blocking-gate.md</Path>` | macOS x64/sleep/descriptor/package/startup residual 全部通过 | T-27, T-28, T-29 | deep | critical | yes | macos-gate-owner | AC-009—AC-010、AC-012—AC-023、AC-027、AC-029—AC-031 | W1 / G2 | blocked |
| T-25 | `<Path>{roots.state}/specdev/changes/{change}/ticket/25-final-umbrella-acceptance.md</Path>` | final SHA 上 31 AC、15 DoD、质量、结构与 Evidence 得到 verdict | T-22, T-23 | deep | critical | yes | final-acceptance-owner | AC-001—AC-031 | W2 / G3 | blocked |
...
T-27 [DONE: G1 product fingerprint frozen]
  ...
  └─→ T-29 [DONE: secure-write pipe EOF]
          ...
          └─→ T-23 [BLOCKED: macOS environment residuals]
          T-22 ─┐
                ├─→ T-25 [READY AFTER BOTH: final verdict]
          T-23 ─┘
```

- **根节点：** T-27。
- **并行扇出：** T-28/T-29 已完成；T-22 已在包含两项修复的固定点通过。T-23 仍等待真实 macOS x64、physical sleep/wake 和 literal descriptor 环境。
- **关键汇合：** T-25；任何平台 Evidence 过期、skip 或失败都阻止开始。
...
- 最大并发为 3，来自 `<Path>{roots.state}/specdev/config.json</Path>`；本计划实际最大项目写并发为 2。
- T-27 是 root manifest、runtime verifier、Desktop startup/readiness 的唯一 shared owner。
- T-22 与 T-23 在依赖关闭后可并行，写路径分别限于 Windows/macOS platform harness，无交集。
- T-25 仅文档/只读验收，不修改项目路径。
- 并行写代码使用独立 worktree；Evidence 和 change 状态按工件 owner 顺序同步，不并发覆盖。
...
T-27 的 G1 子门曾关闭，随后 T-28 暴露 NSIS 栈安全，T-29 又由 Windows Knowledge 写入诊断出 stdin EOF 错误码缺陷；两项已在本地固定点修复并验证。T-22 已基于最终本地产品指纹完成；T-23 仍因真实 macOS 环境缺失 blocked。commit 未获授权，因此固定点使用 base HEAD + 可重算产品/测试路径指纹。任一后续产品代码变化都会使平台 Evidence 失效。
