...
- [ ] `AC-003`、`AC-006`、`AC-007`、`AC-008` 全部满足。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-08-27-macos-release-team-id-crash/evidence/T-01.md</Path>`。
- [ ] 实际项目修改未超出 `writable_paths`。
- [ ] Ticket 形成非空 implementation commit，direct-parent 验证通过且父分支 result 已记录。
- [ ] T-02 尚未执行的原始平台 E2E 不被表述为通过。
- [ ] 未发生未批准的范围、契约或发布偏差。
