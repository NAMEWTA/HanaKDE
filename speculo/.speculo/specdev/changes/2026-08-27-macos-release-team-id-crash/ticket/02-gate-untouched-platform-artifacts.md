...
- [ ] `AC-001`、`AC-002`、`AC-004`、`AC-005`、`AC-009`、`AC-010` 全部满足。
- [ ] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/2026-08-27-macos-release-team-id-crash/evidence/T-02.md</Path>`。
- [ ] 实际项目修改未超出 `writable_paths`，shared path 由 T-02 修改。
- [ ] Ticket 形成非空 implementation commit，direct-parent 验证通过且父分支 result 已记录。
- [ ] required E2E 在真实目标平台完成；没有用重签副本、静态 `codesign --verify` 或构建成功冒充用户启动成功。
- [ ] 未发生未批准的范围、契约或发布偏差。
