...
status: done
planning_depth: deep
planning_depth_reason: 扩展持久 Todo/Project schema、显式上下文继承写入规则和 Project 生命周期数据完整性合同。
ready: true
risk: medium
blocked_by: [T-02]
...
- 首版不实现自然语言识别；日期、Project、tag、提醒和执行模式只能通过显式控件/chip 设置。多行粘贴不拆分、不合并、不部分创建。
- Project/List 自身可进入 Trash；不级联删除 Todo。未完成 Todo 在活动投影中转入 Inbox，并保留历史 projectRef；恢复 Project 是独立动作，不能自动重建或抢回已重新分配的 Todo。
...
|---|---|---|
| Inbox、Project/List CRUD、tags、显式 inherit chips、逐项捕获、筛选和 Project Trash/恢复投影 | T-01 store/routes/tools/Page，T-02 version/confirmation/undo 模式 | 自然语言识别、批量创建、嵌套项目、协作、时间 DST 语义、周期和自动化 |

## 4. 要构建什么

用户在任何主视图逐项键入标题时，Page 只把当前可见的 Today/Project/date context 渲染为可移除 chip；全局入口没有上下文时写入 Inbox，移除 chip 后不写入继承值。多行粘贴提示逐项添加。Project 列表支持创建、重命名、过滤、Trash 和恢复；Project 进入 Trash 后未完成 Todo 转 Inbox 投影并保留历史引用。route 与 tool 对同一组织 DTO、version 和错误规则达成一致。

## 5. 实现契约

- **入口或接缝：** capture application service、Project routes/tools、Page quick capture 与导航过滤器。
- **输入与输出：** capture 输入为单条原文本、显式 context 和用户确认后的 chips；输出为规范 Todo；Project mutation 接收 id/version/name/lifecycle。
- **公共接口变化：** 新增插件内 Project/tag 字段、routes 和 namespaced tools；无宿主变化。
- **不变量：** 一个 Todo 最多一个 Project；Project Trash 不删除 Todo，活动未完成 Todo 转 Inbox 且历史 projectRef 可审计；无 NLP/隐藏字段；tag 查询有界。
- **状态或数据流：** raw input -> visible explicit context chips -> confirmed single-item DTO -> application service/store；Project Trash 与活动投影变更同事务，恢复 Project 时检查 Todo 当前引用版本。
- **错误与失败行为：** invalid_context、duplicate_project、stale_version、multiline_capture_rejected、project_restore_conflict、transaction_failed 可判定；失败不暗写字段、不产生部分 Todo。
- **兼容要求：** 既有无组织 Todo 解释为 Inbox；T-01/T-02 DTO 和生命周期保持。
- **安全与隐私要求：** 捕获文本仅作为 Todo 用户数据处理，不进入诊断；不向外部服务发送自然语言。

## 6. 执行路线

1. 用逐项 capture、IME、多行粘贴拒绝、Project Trash/恢复和冲突测试固定组织不变量与失败结果。
2. 事务扩展私有 schema/DTO，并实现 Project/List、tag 与 Inbox 查询。
3. 实现显式 context -> chips -> 单项 create 流程及对应 routes/tools，禁止 NLP、批量创建和隐藏继承。
...
| 失败路径 | capture/transaction 故障 | 测试 IME Enter、多行粘贴、未显示/移除 chip、stale Project、Trash/恢复冲突 | 不暗写字段、不拆分创建，失败时 Project/Todo 均保持原状 | `<Path>{roots.state}/specdev/changes/2026-08-09-internalize-todolist-plugin/evidence/T-03.md</Path>` |
...
- [x] AC-006：逐项捕获、IME/multiline 规则和显式 chip 决定写入，缺失或移除 chip 不暗写，首版无 NLP。
- [x] AC-007：单层 Project/List 与 tags 可用，Project Trash 不删除 Todo，活动 Todo 转 Inbox 并可在 Project 恢复后按引用版本重新解析。
- [x] AC-029、AC-031：失败稳定无部分写入，核心捕获/组织交互在五语言、键盘和窄布局下可用。
- [x] Evidence 写入 `<Path>{roots.state}/specdev/changes/2026-08-09-internalize-todolist-plugin/evidence/T-03.md</Path>`，产品 diff 仅在插件根。
