...
risk: high
blocked_by: []
contract_ids: [AC-001, AC-002, AC-003, AC-004, AC-032]
...
- **可观察产出：** Hana 能发现插件并打开 page/widget；首页显示 A/HK 模块能力矩阵；插件 route/tool 能返回版本化 `AssetRef`、`ProviderCapability`、`DataSnapshot`、`ResearchRun`、`EvidenceRef` 和结构化错误的空实现/fixture。
- **来源：** `US-001`、`US-015`、`AC-001`～`AC-004`、`AC-032`、`ADR-001`、`ADR-003`、`CODE`、`RESEARCH`。
...
- manifest 预声明本 Spec 所需的最小 SDK capability：`network.fetch`、ResourceIO 读/搜/写/物化、`session`、`agent`、`model.sample`；实际外部 host 仍由 provider 配置和 probe 决定。
...
|---|---|---|
| manifest、page/widget、插件入口、领域类型、状态枚举、错误 envelope、诊断占位和插件内契约测试 | Hana 插件发现、WebView、`hana.api.fetch`、SDK capability 和 theme | 真实 provider、量化计算、持仓数据、宿主 API、全局 registry、交易工具 |
...
2. 建立共享领域 schema、能力状态枚举、错误 envelope、版本化私有 store 接缝和最小 fixture。
...
|---|---|---|---|---|
| 正常路径 | plugin manifest/route harness | 运行插件 manifest、surface、capability fixture 测试 | plugin 可发现、page/widget 可加载，矩阵包含 A/HK 全模块 | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-01.md</Path>` |
...
| UI E2E（owner：当前执行 owner） | page/widget accessibility | 读取桌面/窄屏可访问性树并打开首屏 | 所有模块入口和状态可见且不重叠 | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-01.md</Path>` |
...
- [ ] `AC-001`～`AC-004`、`AC-032`：插件可发现、可停用、路径无越界，首页能力矩阵和共享 schema 可用。
