...
|---|---|---|---|---|---|---|---|
| T-01 | 真实宿主 task backend ready；Todo/Project CRUD 和完整关联能力通过 | — | medium | yes | root | AC-001—AC-006 | in_progress |
| T-02 | Todo 继承系统设计语言；AI 工具目录完整且可验证 | T-01 | medium | yes | root | AC-007—AC-008 | in_progress |

单 Ticket 由 root 在 current workspace 串行完成；没有并行 writer 或共享路径争用。

## 3. 依赖 DAG

`T-01 -> T-02 -> repository gates -> pending authorized integration`
