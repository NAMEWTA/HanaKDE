...
- 每次测量使用独立临时 HANA_HOME 和本地 SSD 临时目录；搜索 warm 场景先完成一次 query。搜索必须分别记录 1—2 code-point 短查询扫描与 3+ code-point trigram 查询，不能用后者掩盖前者。
...
| 搜索 warm（trigram） | 100k resources / 4 sources，3+ code points | p50 ≤ 150ms；p95 ≤ 500ms；取消响应 ≤ 100ms |
| 搜索 warm（短查询） | 100k resources / 4 sources，1—2 code points | 每来源首批 50 条 p95 ≤ 1.2s；可分页；取消响应 ≤ 100ms；结果仍须符合连续子串 |
