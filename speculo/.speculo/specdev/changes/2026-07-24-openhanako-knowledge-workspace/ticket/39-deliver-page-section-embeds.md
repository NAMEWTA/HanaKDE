...
- **当前现状：** 当前实现接缝位于 `desktop/src/react/editor/`、`lib/knowledge-workspace/link-resolver.ts`；本 ticket 只扩展这些公开边界。
...
- [ ] 循环 key 使用完整 KnowledgeResourceAddress；嵌入不重复进入宿主索引；内部链接按源页面解析。
