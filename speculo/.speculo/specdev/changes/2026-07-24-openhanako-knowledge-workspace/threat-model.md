...
| TM-007 | Renderer | 恶意 Mermaid | 渲染图表 | 配置注入或过期任务覆盖 | fixed config + isolated worker + cancellation | malicious config + race tests | 14,33 | Mermaid library vulnerabilities需依赖更新 |
| TM-008 | 内存/CPU | 恶意大文件 | 打开/索引 | 压缩炸弹、超大媒体、巨型文本 | Content Gate + limits + cancel | 10MiB boundary/bomb fixtures | 13,14,17,42 | metadata读取仍有有限成本 |
...
| TM-011 | 索引正确性 | 事件丢失/乱序 | watcher不可靠 | stale/duplicate index | sequence + disk reread + reconcile | gap/replay/burst/duplicate | 43 | 极端持续 churn 可保持 stale 状态 |
| TM-012 | 复合操作 | 进程崩溃 | mutation进行中 | 半移动、半重写或丢失结果 | durable journal + recovery barrier | 每个 named failure point restart | 10,54,55,56 | RECOVERY_REQUIRED 需用户处理 |
| TM-013 | Native bridge | 恶意 Renderer | 可调用 IPC | 任意本机路径操作 | single-use grant + action/window binding | replay/wrong action/expired grant | 14,51,56 | Electron Main 被攻陷不在应用层防护范围 |
| TM-014 | 导入 | 恶意外部目录 | picker/clipboard | symlink、特殊文件、无限目录 | Main-to-Server plan + content gate + limits | symlink/device/deep tree/cancel | 14,51 | 外部文件读取权限由 OS 决定 |
...
| TM-018 | 测试数据 | CI/开发机 | 自动化运行 | 读取或破坏真实 home | isolated temp HANA_HOME/workspace | assert paths under temp root | 01,14,57 | OS native integration仍需平台 runner |

## 发布要求

- TM-001—TM-018 全部必须有实际通过或明确未执行状态。
...
- 新增 trust boundary 必须先扩展本矩阵、ADR/LOG/CONTEXT 与对应 ticket。
