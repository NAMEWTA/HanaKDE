...
## 11. 用户取消决定（2026-08-22）

用户确认当前没有真实 macOS x64、物理 sleep/wake 和 literal kernel descriptor 环境，并明确要求不再执行这些测试、直接进入归档。因此本 Ticket 以 `cancelled` 终止，不把缺失 Evidence 表述为通过。已有 macOS arm64、DMG/direct-flow、watcher lease 和 deterministic reconcile 结果保留在 Evidence 中；未执行的 x64、物理睡眠和 literal descriptor 行作为已知残余风险随归档保留。
