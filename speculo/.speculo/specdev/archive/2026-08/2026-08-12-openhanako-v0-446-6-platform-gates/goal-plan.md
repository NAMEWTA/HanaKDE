...
成功要求 T-27、T-28、T-29、T-22、T-23、T-25 全部 done，三个 Gate 均有 final-SHA Evidence。以下均是假完成：普通 `npm install` 后未验证包内入口；只让当前 typebox import 成功；开发启动靠等待第二次成功；只有 unit fixture 没有 packaged repair E2E；Windows/macOS 任一 blocking skip；T-25 使用 G1 关闭前的旧 Evidence。

2026-08-22 用户明确覆盖上述平台完成门：T-23 的真实 macOS x64、物理 sleep/wake、literal descriptor 与依赖重跑被取消，T-23 以 `cancelled` 终止；T-25 可基于真实 Windows、macOS arm64 和当前回归给出 accepted-with-waiver verdict。取消项不表述为 pass。

### Non-goals

不升级依赖或改 lock，不自动修复开发依赖，不改变 artifact protocol/签名/发布，不迁移/修改真实用户数据，不以 Linux 或 mock 替代阻断平台，不执行 push、PR、remote merge 或发布动作。
...
```text
W0 / G1                                  W1 / G2                         W2 / G3
T-27 startup fix → T-28 NSIS → T-29 EOF ─┬─→ T-22 real Windows ─┐
                                            │                       ├─→ T-25 final acceptance
                                            └─→ T-23 macOS residual ─┘
```

关键路径为 `T-27 → T-28 → T-29 → max(T-22, T-23) → T-25`。`current` 策略下所有 Ticket 严格串行；T-22/T-23 的平台 Evidence 可在隔离环境分别采集，但不形成第二个当前 workspace writer。T-25 是只读汇合。
...
| W0 | T-28 | T-27 done | NSIS installer hook 与 contract test | 无 | G1 子门 |
| W0 | T-29 | T-27/T-28 done | Windows secure-write helper 与 contract test | 无 | 关闭 G1 |
...
| T-28 | `<Path>{roots.state}/specdev/changes/{change}/ticket/28-windows-installer-stack-safety.md</Path>` | NSIS hook 栈安全 | deep/critical | T-27 | W0/G1 | windows-installer-owner | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-28.md</Path>` |
| T-29 | `<Path>{roots.state}/specdev/changes/{change}/ticket/29-windows-secure-write-pipe-eof.md</Path>` | secure-write stdin EOF 收敛 | deep/critical | T-27, T-28 | W0/G1 | secure-write-owner | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-29.md</Path>` |
| T-22 | `<Path>{roots.state}/specdev/changes/{change}/ticket/22-windows-blocking-gate.md</Path>` | Windows native/package/startup pass | deep/critical | T-27, T-28, T-29 | W1/G2 | windows-gate-owner | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-22.md</Path>` |
| T-23 | `<Path>{roots.state}/specdev/changes/{change}/ticket/23-macos-blocking-gate.md</Path>` | macOS residual/package/startup pass | deep/critical | T-27, T-28, T-29 | W1/G2 | macos-gate-owner | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-23.md</Path>` |
...
- 六个 Ticket 均 done，Evidence 包含实际修改/命令/结果/失败分类/风险/SHA/cleanup。
...
|---|---|---|---|---|---|
| G1 Runtime Integrity Stable | T-27 Ready；诊断红灯可重复 | T-27/T-28/T-29 done/Evidence；残缺 fixture 红绿；Pi import；dev/package/optional JSON tests；NSIS stack 与 secure-write EOF 合同；quality/build/start smoke；lock/version diff clean | T-22、T-23 | startup-integrity-owner；用户已批准恢复策略 | 产品/合同失败返回 T-27/T-28/T-29 owner；高影响偏差回 Spec/ADR；不开放 W1 |
...
| T-28 | T-27 done | windows-installer-owner | NSIS contract 与 Windows installer Gate | T-28 Evidence | stack contract 通过 |
| T-29 | T-27/T-28 done | secure-write-owner | native helper contract 与 Windows Knowledge write | T-29 Evidence | EOF/write convergence 通过 |
...
| 动作 | 状态 | 目标与条件 |
|---|---|---|
| Current workspace Ticket changes | allowed | 用户要求在当前分支处理；仅各 Ticket `writable_paths`，严格串行 |
| Ticket worktree local changes | not-authorized | 用户明确禁止开启 worktree |
| Local tests/build/package | allowed | 使用隔离 fixtures/Profile；真实平台安装只作用于明确测试目标 |
| Implementation commit | allowed | 用户 2026-08-22 要求完成 Tickets 并在当前分支操作 |
| Local direct-parent verification and parent update | allowed | 仅 `hanakde` 当前分支；Lead 核对后推进 |
| Local candidate integration and parent update | not-authorized | `current` 策略不使用 candidate/worktree |
| Push / PR / remote merge | not-authorized | 不操作远程或 PR |
| Branch/worktree cleanup | not-authorized | 本计划未创建 worktree，也不删除来源 ref |
...
每个 Ticket 完成或阻塞时写入对应 Evidence，记录 fixed SHA、平台/架构、实际命令、退出状态、失败分类、路径审计、cleanup 和恢复条件。状态同步顺序为 Ticket frontmatter → Tickets Map → Goal Plan current status → change status。current workspace 中 Ticket 严格串行；平台 Evidence 可在隔离环境分别采集，但 G2 只在两份 Evidence 都指向 G1 result 且无产品 diff 时关闭。
...
WAVE_STATUS wave=W1 ready=none active=none done=T-22 cancelled=T-23 blocked=none
GATE_STATUS gate=G2 state=closed evidence=<Path>{roots.state}/specdev/changes/{change}/evidence/T-22.md</Path>,<Path>{roots.state}/specdev/changes/{change}/evidence/T-23.md</Path> risks=macOS-x64-sleep-descriptor由用户明确waive且不声明pass
WAVE_STATUS wave=W2 ready=none active=none done=T-25 blocked=none
GATE_STATUS gate=G3 state=closed evidence=<Path>{roots.state}/specdev/changes/{change}/evidence/T-25.md</Path> risks=accepted-with-explicit-platform-evidence-waiver
```

### Pending Decisions and Blockers

产品和恢复策略未决问题为零。G1 已关闭；T-28/T-29 产品修复和 T-22 Windows Gate 的原始采集基线为 `61dcfcba` 加内容指纹 `485ABFA47F334672A07845500260A07E922AE79491A0EA9F30A0B411AD252EB1`，正式 NSIS、静默安装、installed direct flow 与确认 repair 均通过。上述实现和 Evidence 已在当前 `hanakde` 分支以 direct-parent 固定到 `f29abef4a7a79ac9eefebe0ed4597f1252a2b29c`；本机 Node 24 arm64 复核通过目标测试、typecheck、runtime dependency、client build 和 macOS arm64 runner，代码标准轴无新增 finding。用户于 2026-08-22 取消不可用的 macOS x64、物理 sleep/wake、literal descriptor 与依赖重跑；T-23 为 cancelled，T-25 以 approved waiver 完成，G2/G3 关闭。
...
TICKET_STATUS id=T-23 state=cancelled evidence=<Path>{roots.state}/specdev/changes/{change}/evidence/T-23.md</Path> deviation=D-T25-02-approved
TICKET_STATUS id=T-25 state=done evidence=<Path>{roots.state}/specdev/changes/{change}/evidence/T-25.md</Path> deviation=D-T25-02-approved
```
