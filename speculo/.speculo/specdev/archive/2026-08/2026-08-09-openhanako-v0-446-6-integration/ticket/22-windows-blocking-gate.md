...
> **拆分记录（2026-08-12）：** 按用户要求，本 Ticket 的未完成 Windows 原生阻断工作已移入后续 change `<Path>{roots.state}/specdev/changes/2026-08-12-openhanako-v0-446-6-platform-gates/</Path>`，原 umbrella change 不再等待此 Ticket。原 Evidence 保留 `blocked` 事实，不将平台缺失伪装为通过。
