...
- [x] `AC-001`—`AC-005` 全部满足。
- [x] local 与 mount 都加载当前工作区，切换后无旧数据污染。
- [x] remote/越界路径测试 fail closed。
- [x] source 失败不再伪造 available main。
- [x] focused tests 与 typecheck 通过并记录 Evidence。
- [x] 修改未超出 writable paths；后续明确授权的 implementation/direct-parent result 为 `b0c74282`。
