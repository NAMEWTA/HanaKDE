...
- [x] AC-007、AC-010、AC-014：导入、显式写回和 ResourceRef 安全边界通过；AC-009 由 T-02/T-03 的 renderer/media Evidence 覆盖。
- [x] 所有 ResourceIO 正常/拒绝/冲突证据写入 `<Path>{roots.state}/specdev/changes/2026-08-13-markdown-wechat-plugin/evidence/T-04.md</Path>`。
- [x] 不访问绝对路径，不绕过 ResourceIO，不发生隐式 workspace write。
