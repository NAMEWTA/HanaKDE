...
- [x] AC-001、AC-002、AC-011、AC-012：插件加载、Page/Widget surface、私有 envelope 和同源恢复通过。
- [x] 正常/失败/UI E2E/回归验证全部写入 `<Path>{roots.state}/specdev/changes/2026-08-13-markdown-wechat-plugin/evidence/T-01.md</Path>`。
- [x] 实际修改不超出 `<Path>plugins/markdown-wechat/**</Path>`。
- [x] 无未批准宿主 API、数据迁移或范围偏差。
