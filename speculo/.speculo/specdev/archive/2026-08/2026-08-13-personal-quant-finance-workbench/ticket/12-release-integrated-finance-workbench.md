...
- [x] `AC-001`～`AC-038` 全部有前序 Ticket Evidence 和本票集成证据，无 deferred/uncovered。
- [x] 生产实现和测试 fixture 只在 `<Path>plugins/finance-workbench/</Path>`，宿主/其他插件无金融代码。
- [x] plugin.dev、构建、UI、隐私、安全、数据正确性、任务恢复和无交易副作用门全部通过。
- [x] 验证矩阵记录到 `<Path>{roots.state}/specdev/changes/{change}/evidence/T-12.md</Path>`，无未批准偏差。
