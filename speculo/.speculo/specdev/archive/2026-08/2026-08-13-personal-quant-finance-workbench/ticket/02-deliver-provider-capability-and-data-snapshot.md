...
- [x] `AC-004`、`AC-006`、`AC-008`、`AC-030`、`AC-034`、`AC-035`、`AC-037`：BYOK 探测、逐数据集选源、可信快照、失败降级、本地原型门和结构化错误可重复验证。
- [x] provider 未通过许可/质量/PIT/单位门时不会显示 supported。
- [x] 验证矩阵记录到 `<Path>{roots.state}/specdev/changes/{change}/evidence/T-02.md</Path>`，修改只在授权插件路径。
