...
contract_ids: [AC-017, AC-018, AC-019, AC-020, AC-036]
owner: root
...
- **目标：** 在 T-07 策略定义和 T-02 数据快照上交付 A/HK 规则感知、来源冻结、可解释、可恢复的回测。
- **可观察产出：** 用户在执行前看到交易日历、T+1/涨跌停、PIT、复权、费用、滑点、流动性、容量和 RunSourceManifest 门；结果包含收益/风险/成本/样本覆盖、冻结来源与研究免责声明。
- **来源：** `US-011`、`US-012`、`US-020`、`AC-017`～`AC-020`、`AC-036`、`ADR-005`、`DEC-011`、`INV-01`、`INV-06`。
...
- 回测启动时必须冻结 RunSourceManifest；结果必须携带 StrategyDefinition、DataSnapshot、RunSourceManifest、cost manifest、运行时、错误/缺失、容量和随机性/确定性标记。
- 运行中 provider/source kind、adapter version、schema hash、snapshot lineage 或 SourcePolicy version 不匹配时暂停/失败并要求恢复原 manifest 或新建 run，不静默续接。
...
用户选择不可变策略版本、A/HK Universe 和数据区间，系统先展示质量门与 RunSourceManifest 并要求确认。门禁通过后执行回测任务，显示进度、可取消/恢复状态；恢复时严格核对 manifest。完成后展示收益、风险、回撤、换手、成本影响、流动性/容量限制、缺失统计、冻结来源、EvidenceRef 和免责声明。时间旅行、幸存者偏差、错误复权/单位或来源漂移 fixture 会被阻断。

## 5. 实现契约

- **入口或接缝：** backtest route/tool、quality gate evaluator、engine adapter、result store、result page。
- **输入与输出：** 输入 StrategyDefinition version、DataSnapshot set、RunSourceManifest、market rules、cost/slippage/liquidity/capacity assumptions、run budget；输出 ResearchRun/metrics/source+cost manifests/checkpoint 或稳定阻断错误。
- **公共接口变化：** 插件内 backtest request/result schema；不改宿主任务 API。
- **不变量：** no look-ahead、PIT/规则/费用门禁、不可变输入、不可变来源清单、幂等 run、无交易副作用；结果没有证据或来源 lineage 不能标可信。
...
3. 在 run 创建时冻结 RunSourceManifest，实现任务状态、manifest-aware checkpoint、幂等 result store 和 metrics/provenance。
...
| 失败路径 | gate/fault injection | 缺 PIT/日历/成本/容量，注入未来字段、错误复权、来源/adapter/schema/lineage/policy 漂移、取消/worker 失败 | blocked/failed/cancelled/recoverable，无换源续跑、部分可信结果和副作用 | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-08.md</Path>` |
...
| UI E2E（owner：Lead） | backtest page | 桌面/窄屏确认门禁、取消、查看结果 | 所有状态和免责声明可见、无重叠 | `<Path>{roots.state}/specdev/changes/{change}/evidence/T-08.md</Path>` |

- **Workspace checks（current-workspace）：** implementation owner 在 current workspace 运行 deterministic engine、市场规则/成本/PIT gate、偏差 fixture、取消恢复、结果 schema、类型检查和插件构建，不创建 source worktree。
- **E2E disposition：** required：回测从门禁确认、任务执行到结果展示跨越 UI、route、worker、持久化和金融计算边界，时间旅行或成本遗漏属于高事故半径错误。
- **E2E owner/environment：** Lead / current-workspace；在 direct-parent 状态以固定数据运行 A/HK 正常回测，并验证缺 PIT/日历/成本/容量、未来字段、取消与恢复，预期不合格运行 blocked，合格结果完整冻结来源、假设、成本和免责声明。
- **Integration evidence（direct-parent）：** 记录 implementation commit、parent before SHA、固定 fixture/hash、任务与 UI E2E 结果、父分支 result SHA 及包含关系。
...
- [x] `AC-017`～`AC-020`、`AC-036`：回测门禁、来源冻结/恢复核对、规则、成本、偏差检测、结果解释和无副作用成立。
