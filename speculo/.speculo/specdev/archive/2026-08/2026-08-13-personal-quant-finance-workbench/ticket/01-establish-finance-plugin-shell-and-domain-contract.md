...
- [x] `AC-001`～`AC-004`、`AC-032`、`AC-034`～`AC-037`：插件可发现、可停用、路径无越界，首页能力矩阵、敏感 provider 配置和共享来源 schema 可用；本地 source 默认不超过 experimental/blocked。
- [x] 验证矩阵全部执行并记录到 `<Path>{roots.state}/specdev/changes/{change}/evidence/T-01.md</Path>`。
- [x] 后续 Ticket 只能读取本票 owner 的 manifest/domain 契约，未发生未批准的宿主扩展。
- [x] 实际修改未超出 `<Path>plugins/finance-workbench/**</Path>`，Ticket、Map 和 Evidence 状态一致。
