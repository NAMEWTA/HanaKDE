...
id: T-51
title: "交付外部导入与原生 Picker"
status: done
planning_depth: standard
planning_depth_reason: "历史完成 Ticket 的 SpecDev v3 兼容迁移；保留既有实现、验证与验收记录。"
ready: false
risk: medium
blocked_by: ["T-01","T-03","T-04","T-06","T-10","T-14","T-48"]
contract_ids: ["KW-US-165","KW-US-180","KW-US-181","KW-RULE-OP","KW-RULE-SEC","KW-RULE-IMPORT","KW-RULE-NATIVE"]
owner: historical-implementer
expected_changes: ["<Path>{roots.state}/specdev/changes/2026-07-24-openhanako-knowledge-workspace/ticket/51-deliver-import-native-picker.md</Path>"]
writable_paths: []
read_only_paths: []
shared_paths: []
shared_path_owners: []
...
- [x] 远程端不接收本机路径；错误/缺失 native credential、普通 server token 和 replay 均被拒绝；取消不写入；批次结果确定；symlink 与超限输入按威胁模型拒绝。
...
- **实现结果：** opaque File handle、一次性 owner/window/action/address grant、Main-only credential、picker/import/reveal 与 Open 降级已交付。
- **精确自动化：** `native contract/import/grant/route security/client` 相关测试包含于最终聚合命令，23 files、65/65 tests 通过；E2E-KW-017 已在 macOS arm64 的实际 Playwright project 中通过。
