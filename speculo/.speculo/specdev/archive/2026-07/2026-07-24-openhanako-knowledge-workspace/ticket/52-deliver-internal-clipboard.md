...
ready: false
risk: medium
blocked_by: ["T-10","T-14","T-38","T-48"]
contract_ids: ["KW-US-182","KW-US-183","KW-RULE-OP","KW-RULE-CLIPBOARD"]
owner: historical-implementer
expected_changes: ["<Path>{roots.state}/specdev/changes/2026-07-24-openhanako-knowledge-workspace/ticket/52-deliver-internal-clipboard.md</Path>"]
writable_paths: []
read_only_paths: []
shared_paths: []
shared_path_owners: []
...
| IN | REUSE | OUT |
|---|---|---|
| 内部剪贴板保存 KnowledgeResourceAddress 与 intent；系统剪贴板只承载可审计外部格式。 | `desktop/src/react/components/desk/DeskTree.tsx`<br>`desktop/preload.cjs` | 未列入本 ticket 的后续功能；修改生成 bundle；创建平行文件系统、编辑器内核或私有 route 捷径 |
...
- `desktop/src/react/__tests__/stores/knowledge-clipboard-slice.test.ts`

**Playwright 用户流程：** 适用；运行 E2E-KW-018

## 验收标准

- [x] 跨来源 cut 被拒绝并提示 copy；复制正文/字节原样；粘贴按结构化批次结果更新选择。
- [x] KW-US-182/183 由 copy/cut、同源/跨来源、确定性冲突名、部分完成与 source 丢失测试直接证明。
...
- **实现结果：** 会话内 copy/cut/paste 状态、同源 move 与跨来源逐字节 copy、部分失败和源保留语义已交付。
- **精确自动化：** `knowledge-copy-service、knowledge-clipboard-slice` 相关测试包含于最终聚合命令，23 files、65/65 tests 通过；E2E-KW-018 已在 macOS arm64 的实际 Playwright project 中通过。
