...
describe("MCP first-class routes", () => {
  ...
  it("does not mount the removed /api/plugins/mcp alias", async () => {
    const mcp = fakeMcp();
    const app = createApp(mcp);

    const alias = await app.request("/api/plugins/mcp/state");

    expect(alias.status).toBe(404);
    expect(mcp.getState).not.toHaveBeenCalled();
  });
  ...
  it("accepts the OAuth callback on the first-class path", async () => {
    ...
    const res = await app.request("/api/mcp/oauth/callback?state=st_1&code=code_1");

    expect(res.status).toBe(200);
    expect(await res.text()).toContain("Connector connected");
    expect(mcp.completeOAuth).toHaveBeenCalledTimes(1);
  });
  ...
  it("does not mount the removed servers route aliases", async () => {
    const mcp = fakeMcp();
    const app = createApp(mcp);
    ...
    expect(mcp.startOAuth).not.toHaveBeenCalled();
  });

  it("does not intercept the removed plugin namespace", async () => {
    const mcp = fakeMcp();
    const app = createApp(mcp);
  ...
  describe("session-scoped permission grants", () => {
    ...
    it("rejects the removed legacy alias", async () => {
  ...
  describe("bulk connector import", () => {
    ...
    it("rejects bulk imports through the removed legacy alias", async () => {
  ...
  describe("cancellable OAuth waits", () => {
    ...
    it("rejects the removed legacy and servers aliases", async () => {
      ...
      for (const routePath of [
        ...
        expect((await app.request(routePath, { method: "POST" })).status).toBe(404);
      }
    });
