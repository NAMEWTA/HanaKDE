...
describe("mobile workbench route", () => {
  ...
  it("uses selectedAgentId only as an authorization target and redacts remote bootstrap paths", async () => {
    ...
    expect(bootstrapPayload).toMatchObject({
      homeFolder: null,
      cwdHistory: [],
      agents: [{ id: "hana", homeFolder: null, effectiveHomeFolder: null }],
    });
    expect(JSON.stringify(bootstrapPayload)).not.toContain(workspace);
  });
  ...
  it("copies tree items inside a mounted workspace without removing the source", async () => {
    tmpDir = makeTmpDir();
    const workspace = path.join(tmpDir, "workspace");
    const mountRoot = path.join(tmpDir, "mounted-docs");
    const hanakoHome = path.join(tmpDir, "hana");
    ...
    expect(res.status).toBe(200);
    expect(fs.readFileSync(path.join(mountRoot, "notes", "draft.md"), "utf-8")).toBe("draft");
    expect(fs.readFileSync(path.join(mountRoot, "archive", "draft.md"), "utf-8")).toBe("draft");
  });
