...
describe("login item settings", () => {
  it("uses a dedicated login-start argument on Windows so startup can stay hidden", async () => {
    ...
    expect(getLoginItemOptions("win32", "C:\\Program Files\\HanaKDE\\HanaKDE.exe")).toEqual({
      path: "C:\\Program Files\\HanaKDE\\HanaKDE.exe",
      args: [START_AT_LOGIN_ARG],
    });
    expect(wasLaunchedAtLogin({
      platform: "win32",
      argv: ["HanaKDE.exe", START_AT_LOGIN_ARG],
      loginItemSettings: {},
    })).toBe(true);
  });
  ...
  it("reports Linux as unsupported without touching Electron login item APIs", async () => {
    ...
    expect(getAutoLaunchStatus({ app, platform: "linux", argv: [], execPath: "/opt/HanaKDE/hana" })).toEqual({
      supported: false,
      openAtLogin: false,
      openedAtLogin: false,
      status: "unsupported",
  ...
  it("writes and re-reads Windows login item settings with the same path and args", async () => {
    ...
    const status = setAutoLaunchEnabled({
      ...
      platform: "win32",
      argv: [],
      execPath: "C:\\HanaKDE\\HanaKDE.exe",
      enabled: true,
    });

    expect(app.setLoginItemSettings).toHaveBeenCalledWith({
      openAtLogin: true,
      path: "C:\\HanaKDE\\HanaKDE.exe",
      args: [START_AT_LOGIN_ARG],
    });
