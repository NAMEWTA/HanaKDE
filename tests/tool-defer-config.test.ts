...
function currentConnector(overrides: Record<string, unknown> = {}) {
  return {
    id: "github",
...
describe("mcp defer configuration", () => {
  ...
  it("drops incomplete policy records while preserving current records and config defaults", () => {
    const config = normalizeMcpConfig({
      enabled: true,
      connectors: [{
        id: "legacy-github",
    ...
    expect(config.connectors[0].trustReadOnlyHint).toBe(false);
    expect(config.connectors[0].pinnedTools).toEqual({});
    expect(config.deferEnabled).toBe(true);
    expect(config.deferThreshold).toBe(10);
  });
