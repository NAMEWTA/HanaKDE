...
import { loadMcpApprovalIndex, resolveMcpApprovalTarget } from '../../components/input/mcp-approval-actions';
...
describe('loadMcpApprovalIndex', () => {
  it('does not fall back to a removed servers state field', async () => {
    vi.mocked(hanaFetch).mockResolvedValue({
      json: async () => ({ servers: CONNECTORS }),
    } as Response);

    await expect(loadMcpApprovalIndex()).resolves.toEqual([]);
  });
});
