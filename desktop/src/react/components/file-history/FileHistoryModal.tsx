/**
 * Dormant upstream File History renderer input. T-06 deliberately does not
 * register its store slice or mount this component; T-13 owns any future
 * production cutover.
 */
...
interface DormantFileHistoryStoreState {
  fileHistoryModal: {
    open: boolean;
...
// This narrows only the dormant module's selector contract. It neither adds
// File History state to StoreState nor creates a production store entrypoint.
const useDormantFileHistoryStore = useStore as unknown as <T>(
  selector: (state: DormantFileHistoryStoreState) => T,
) => T;

export function FileHistoryModal() {
  const { t } = useI18n();
  const modal = useDormantFileHistoryStore(s => s.fileHistoryModal);
  const close = useDormantFileHistoryStore(s => s.closeFileHistoryModal);
  const agentId = useDormantFileHistoryStore(s => s.currentAgentId);
  const nativeRoot = useDormantFileHistoryStore(s => s.deskWorkspaceNativeRoot || s.deskBasePath);
