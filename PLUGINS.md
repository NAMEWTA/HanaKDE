...
2. 打开 HanaKDE → 设置 → 插件，把文件夹拖进安装区（或压缩成 .zip 拖入）
...
`minAppVersion`（可选）声明插件运行所需的最低 HanaKDE 版本。如果当前 app 版本低于该值，插件不会加载，状态标记为 `incompatible`。建议所有插件都声明此字段，避免用户在旧版本上遇到不兼容问题。
