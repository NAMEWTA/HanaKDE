...
2. Open HanaKDE → Settings → Plugins, drag the folder into the install area (or drag a .zip)
...
`minAppVersion` (optional) declares the minimum HanaKDE version required to run the plugin. If the current app version is lower, the plugin will not load and its status is set to `incompatible`. All plugins should declare this field to prevent compatibility issues on older versions.
...
Plugins can register background tasks so HanaKDE can track and abort them. Runtime lifecycle is managed by `TaskRegistry`.
