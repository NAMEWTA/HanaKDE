...
export const PERSISTENT_STORES: readonly StoreDescriptor[] = Object.freeze([
  ...
  defineStore({
    ...
    format: "json",
    schemaSource: runtimeSource("core/preferences-manager.ts", "PreferencesManager defaults, coercion, and durable preferences writer"),
    openEntry: ["new PreferencesManager"],
    migrationEntry: ["core/preferences-manager.ts unreadable-source preservation"],
    firstPossibleOpenPhase: "desktop_bootstrap",
    firstPossibleWritePhase: "first_run_seed",
    compatibility: "PreferencesManager is intentionally permissive and preserves unreadable source bytes before any replacement document is written.",
  ...
  defineStore({
    ...
    firstPossibleOpenPhase: "engine_construct",
    firstPossibleWritePhase: "engine_construct",
    identityContract: "providerId is the durable key; local provider plugins use a filesystem-safe storage identifier derived from that key.",
    siteRules: rules([
  ...
  defineStore({
    ...
    migrationEntry: ["HanaAgent v1 memories.db import"],
  ...
  defineStore({
    ...
    pathKind: "tree",
    format: "mixed-directory",
    schemaSource: directorySource("lib/checkpoint-store.ts", "file-copy checkpoint manifest protocol"),
    openEntry: ["CheckpointStore"],
    epochPolicy: "compatible",
    ...
    siteRules: rules(["lib/checkpoint-store.ts"], "Writes or restores an existing operational checkpoint."),
  }),
  defineStore({
